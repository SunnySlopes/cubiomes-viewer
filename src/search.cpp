...
{
    StructureConfig sconf;
    if (!getStructureConfig_override(Monument, g->mc, &sconf))
        return;
    ...
    // Collect all monument positions in the area
    for (int rz = rz1; rz <= rz2 && mcnt < MAX_INSTANCES; rz++)
    {
        for (int rx = rx1; rx <= rx2; rx++)
        {
            Pos pc;
            if (!getStructurePos(Monument, g->mc, g->seed, rx, rz, &pc))
                continue;
            ...
            // Check viability
            if (!isViableStructurePos(Monument, g, pc.x, pc.z, 0))
                continue;
    ...
    for (int i = 0; i < mcnt; i++)
    {
        for (int j = i + 1; j < mcnt; j++)
        {
            ...
            // Skip if this is a standard quad configuration (160, 80) or (80, 160)
            if ((adx == 160 && adz == 80) || (adx == 80 && adz == 160))
                continue;
            ...
            // Check if at least one monument is within the specified range
            bool p1_in_range = (p1.x >= x1 && p1.x <= x2 && p1.z >= z1 && p1.z <= z2);
            bool p2_in_range = (p2.x >= x1 && p2.x <= x2 && p2.z >= z1 && p2.z <= z2);
            ...
            dinfo.p[0] = p1;
            dinfo.p[1] = p2;
            // Center is the average of two monuments
            dinfo.center.x = (p1.x + p2.x) / 2;
            dinfo.center.z = (p1.z + p2.z) / 2;
            dinfo.dx = dx;
            dinfo.dz = dz;
            out->push_back(dinfo);
        }
